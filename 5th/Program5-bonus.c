...
void quicksort( std_t in[], int left, int right )
{
	...
	while ( 1 )
	{
		while ( i <= right )		//不超過界限
		{
			if ( (in[i].score < in[left].score) || ((in[i].score == in[left].score) && (in[i].id > in[left].id)) )
				break;
		...
		while ( j > left )		//不低於界限
		{
			if ( (in[j].score > in[left].score) || ((in[j].score == in[left].score) && (in[j].id < in[left].id)) )
				break;
...
int main(void)
{
	...
	int want_order;
	FILE *fp;
	std_t *std = (std_t*)malloc(1000000*sizeof(std_t));
